...
Readme.txt file for Time_Sync Files
----------------------------------------------------

Run file: GetData.m 

The Time_Sync files contain a main script file called GetData.m which calls function files load_fp_file.m and 
load_mocap_file.m

Temporal synchronization of Motion Capture and Force Plates is achieved by utilizing an End of Frame (EOF) signal 
that is generated by the master tracker of the VisualEyez motion capture system and feeding it to the force plates/
Shimmer modules etc. 

A motion capture session consists of a number of frames, each of which in turn has two parts: capture period 
...
Load_fp_file.m also identifies the time at which EOF(2) or (End of Frame signal generated after Frame #2) from the 
